...
namespace AviFile
{
	public class VideoStream : AviStream
	{
        ...
        public VideoStream(int aviFile, Avi.AVICOMPRESSOPTIONS compressOptions, double frameRate, int frameSize, int width, int height, PixelFormat format)
        {
			this.aviFile = aviFile;
			this.writeCompressed = writeCompressed;
			this.frameRate = frameRate;
		...
		private Int16 ConvertPixelFormatToBitCount(PixelFormat format){
            return (short)Bitmap.GetPixelFormatSize(format);            
